...
const ActiveTaskView = ({ data, headers, functions, propertyNames, taskId }: any) => {
    ...
    const [checkboxStates, setCheckboxStates] = React.useState(data.map((item: any) => (
        taskId.includes(item.id)
    )));
    const [selectedTask, setSelectedTask]: any = useState(taskId);
    ...
    const linkTaskHandler = () => {
        const formData: any = {
            taskId: selectedTask
        }
        functions.linkTask(formData);
    }
        ...
        <View style={styles.container}>
                                        ...
                                        <Checkbox
                                            ...
                                            onPress={() => {
                                                if (!selectAll) {
                                                    ...
                                                    if (selectedTask.includes(item.id)) {
                                                        setSelectedTask(selectedTask.filter((taskId: any) => taskId !== item.id))
                                                    }
                                                    else {
            ...
            </View>
            {
                pagedData.length > 0 && <View style={styles.pagination}>
                    <Button mode="contained" onPress={() => linkTaskHandler()}>
                        Link
                    </Button>
